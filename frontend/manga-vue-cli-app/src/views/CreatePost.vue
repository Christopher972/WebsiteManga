<template>
  <div class="create">
    ...
    <h1>Customize ton Post</h1>
   
     <!-- Formulaire de création de publication -->
    <div id ="formCreatePost">
        <form id="form" @submit.prevent="createPost()">
            <label for="title">Name</label>
            <input type="text" name="name" v-model="name" id="title" minlength="8" maxlength= "49" required>
            <label for="description">Description</label>
            <textarea type="text" name="description" v-model="description" placeholder="Open Your Mind..." id="postContent" maxlength="1000"></textarea>
            <label for="imageUrl" class="fileUpload"><i class="fa fa-upload" aria-hidden="true"></i> Download Image</label>
            <input @change ="loadImagePreview()" type="file" ref="file" id="imageUrl" name="imageUrl" accept="image/*">
            <div class="image-preview" v-if="imageLoaded===true">
                <img src="" alt="aperçu de l'image" class="image-preview_image"> 
            </div>
            <label for="heat">Heat</label>
            <input type="range" class="range" min="1" max="10" id="heat" v-model="heat">
            <input type="number" class="number" v-model="heat">   
            <input type="submit" id="postCreate" value="Publier">
        </form>
        <div class ="buttonPost">
            <router-link to="/posts" class= "profile"><i class="fas fa-undo"></i> Back aux Posts </router-link>
        </div>
    </div>
...
methods: {
    ...
    createPost() { /// Fonction appelée lors de la création d'une publication 
        ...
        const post = {"id":this.id, "name":this.name, "description":this.description, "imageUrl":this.imageUrl, "heat":this.heat };
        var data = null;
        if (! imageUrl){
                data = {
                method: 'POST',
...
.create
{
   background-image: url(../assets/fyer.jpg);
}
